...
export interface ZoneData {
  id?: string; // nuevo de Dorian, El backend devuelve la zona completa con id
  name: string;
  description: string;
...
export const getZone = async (
  ...
  if (!res.ok) {
    ...
    throw new Error(errorData.detail?.[0]?.msg || "Error fetching zone");
  }
...
export const getAllZones = async (
  ...
  const res = await fetch(
    `${API_BASE_URL}/zones/?brand_id=${brandId}&campaign_id=${campaignId}`,
    {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    }
  ...
  if (!res.ok) {
    const errorData: ApiError = await res.json();
    throw new Error(errorData.detail?.[0]?.msg || "Error fetching zones");
  }
...
export const getCampaign = async (
  ...
  const url = `${API_BASE_URL}/campaigns/${encodeURIComponent(
    campaignIdPath
  )}?${params.toString()}`;
...
export const makeZonesApi = (brandId: string, campaignId: string) => ({
  ...
  getZone: (zoneId: string) => getZone(brandId, campaignId, zoneId), // ✅ solo zoneId

  getAllZones: () => getAllZones(brandId, campaignId),
  ...
  deleteZone: (zoneId: string) => deleteZone(brandId, campaignId, zoneId), // ✅ solo zoneId
